...
public class Main {
  ...
  public static Bstpair isBst(Node root)
  {
    ...
    mypair.isbst = (leftbst.isbst && rightbst.isbst && (leftbst.max < root.data) && (root.data < rightbst.min));
    mypair.min = Math.min(root.data,Math.min(leftbst.min,rightbst.min));
    mypair.max = Math.max(root.data,Math.max(leftbst.max,rightbst.max));
